...
function populateDropdowns(selectedLoteId) {
  ...
  for (var i = 0; i < globalLotes.length; i++) {
    ...
    if (l.Estado === 'Disponible') {
      optionsCrear += '<option value="' + l.IdLote + '">' + l.Numero + ' · ' + l.DesarrolloNombre + ' · $' + new Intl.NumberFormat('es-MX').format(l.Precio) + '</option>';
    }
  }
  ...
  for (var i = 0; i < globalLotes.length; i++) {
    ...
    if (l.Estado === 'Disponible' || l.IdLote === selectedLoteId) {
      optionsEdit += '<option value="' + l.IdLote + '">' + l.Numero + ' · ' + l.DesarrolloNombre + ' · $' + new Intl.NumberFormat('es-MX').format(l.Precio) + '</option>';
    }
  }
