...
function filtrarLotes() {
  ...
  var filtrados = window.allLotes.filter(function(l) {
    var textMatch = true;
    if (searchQuery) {
      var searchStr = ((l.Numero || l.Número || '') + ' ' + (l.DesarrolloNombre||'') + ' ' + (l.Medidas||'') + ' ' + l.IdLote).toLowerCase();
  ...
  fetchApi('/lotes', { method: 'POST', body: JSON.stringify(payload) })
    .then(function(res) {
      ...
      var cPayload = {
        ...
        MedidaNorte: document.getElementById('lote-medida-norte').value || '',
        Sur: document.getElementById('lote-sur').value || '',
        MedidaSur: document.getElementById('lote-medida-sur').value || '',
        Este: document.getElementById('lote-este').value || '',
        MedidaEste: document.getElementById('lote-medida-este').value || '',
        Oeste: document.getElementById('lote-oeste').value || '',
        MedidaOeste: document.getElementById('lote-medida-oeste').value || '',
        IdLote: res.IdLote
      };
...
function abrirModalEditarLote(id) {
  fetchApi('/lotes/' + id)
    .then(function(lote) {
      ...
      document.getElementById('edit-lote-numero').value = lote.Numero || lote.Número || '';
      ...
      document.getElementById('edit-lote-medida-norte').value = lote.MedidaNorte || '';
      document.getElementById('edit-lote-sur').value = lote.Sur || '';
      document.getElementById('edit-lote-medida-sur').value = lote.MedidaSur || '';
      document.getElementById('edit-lote-este').value = lote.Este || '';
      document.getElementById('edit-lote-medida-este').value = lote.MedidaEste || '';
      document.getElementById('edit-lote-oeste').value = lote.Oeste || '';
      document.getElementById('edit-lote-medida-oeste').value = lote.MedidaOeste || '';
  ...
  fetchApi('/lotes/' + idLote, { method: 'PUT', body: JSON.stringify(payload) })
    .then(function() {
      var cPayload = {
        ...
        MedidaNorte: document.getElementById('edit-lote-medida-norte').value || '',
        Sur: document.getElementById('edit-lote-sur').value || '',
        MedidaSur: document.getElementById('edit-lote-medida-sur').value || '',
        Este: document.getElementById('edit-lote-este').value || '',
        MedidaEste: document.getElementById('edit-lote-medida-este').value || '',
        Oeste: document.getElementById('edit-lote-oeste').value || '',
        MedidaOeste: document.getElementById('edit-lote-medida-oeste').value || '',
        IdLote: idLote
      };
      return fetchApi('/colindancias', { method: 'POST', body: JSON.stringify(cPayload) })
        .catch(function() {
