...
function cargarContratos() {
  ...
  // Cargar listas en paralelo una sola vez al cargar la sección
  Promise.all([
...
function populateDropdowns(selectedLoteId) {
  ...
  var optionsCrear = '<option value="">Seleccionar…</option>';
  for (var i = 0; i < globalLotes.length; i++) {
    var l = globalLotes[i];
    if (l.Estado === 'Disponible') {
      optionsCrear += '<option value="' + l.IdLote + '">' + (l.Numero || l.Número || '') + ' · ' + l.DesarrolloNombre + ' · $' + new Intl.NumberFormat('es-MX').format(l.Precio) + '</option>';
    }
  }
  if (selCrear) selCrear.innerHTML = optionsCrear;

  var optionsEdit = '<option value="">Seleccionar…</option>';
  for (var i = 0; i < globalLotes.length; i++) {
    var l = globalLotes[i];
    if (l.Estado === 'Disponible' || l.IdLote === selectedLoteId) {
      optionsEdit += '<option value="' + l.IdLote + '">' + (l.Numero || l.Número || '') + ' · ' + l.DesarrolloNombre + ' · $' + new Intl.NumberFormat('es-MX').format(l.Precio) + '</option>';
    }
  }
...
function renderContratos(data) {
  ...
  for (var i = 0; i < data.length; i++) {
    ...
    if (role === 'directivo' || role === 'admin' || role === 'administrador') {
        html += '<button class="btn-outline btn-sm" onclick="abrirModalEditarContrato(' + c.IdContrato + ')">Editar</button>';
    }
    if (role === 'directivo' || role === 'admin' || role === 'administrador') {
        html += '<button class="btn-danger btn-sm" onclick="eliminarContrato(' + c.IdContrato + ')">Eliminar</button>';
    }
    html += '</td>';
    html += '</tr>';
  }
...
function guardarContratoModal() {
  ...
  fetchApi('/contratos', {
    ...
    .then(function() {
      showToast('Contrato creado con éxito', 'success');
...
function generarDocumentoContrato(id) {
  // Guardar datos en localStorage para que imprimir-contrato.html los pueda leer
  try {
    localStorage.setItem('sofi-contratos', JSON.stringify(window.allContratos || []));
    localStorage.setItem('sofi-clientes', JSON.stringify(globalClientes || []));
    localStorage.setItem('sofi-lotes', JSON.stringify(globalLotes || []));
