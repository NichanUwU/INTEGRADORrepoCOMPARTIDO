...
function renderTestigos(data) {
  ...
  for (var i = 0; i < data.length; i++) {
    ...
    html += '<td>' + (t.Telefono || t.Teléfono || '') + '</td>';
    html += '<td>' + (t.Ciudad || '') + '</td>';
    html += '<td>' + (t.Estado || '') + '</td>';
...
function guardarTestigo() {
  ...
  if (!idCliente) {
      showToast('Debes seleccionar un Cliente Añociado', 'warn');
      return;
  }
  ...
  fetchApi('/testigos', { method: 'POST', body: JSON.stringify(body) })
    .then(function() {
...
function abrirModalEditarTestigo(id) {
  ...
  document.getElementById('edit-t-direccion').value = t.Direccion || t.Dirección || '';
  ...
  document.getElementById('edit-t-telefono').value = t.Telefono || t.Teléfono || '';
...
function actualizarTestigo() {
  ...
  fetchApi('/testigos/' + id, { method: 'PUT', body: JSON.stringify(body) })
    .then(function() {
