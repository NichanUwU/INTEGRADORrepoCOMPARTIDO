...
function renderEmpleadosTable(page) {
  ...
  paginated.forEach(u => {
    ...
    html += `<tr>
      ...
      <td>${u.Direccion || u.Dirección || ''}</td>
      <td>${u.Telefono || u.Teléfono || 'N/A'}</td>
      <td><span class="chip ${chipEstatus}">${u.Estatus || 'Activo'}</span></td>
      <td style="display:flex;gap:6px;flex-wrap:wrap;">
        <button class="btn-outline btn-sm" onclick="abrirModalEditarEmpleado(${u.IdEmpleado})">Editar</button>
      </td>
    </tr>`;
  });
...
function abrirModalEditarEmpleado(id) {
  ...
  if(e) {
    ...
    document.getElementById('edit-emp-direccion').value = e.Direccion || e.Dirección || '';
    ...
    document.getElementById('edit-emp-telefono').value = e.Telefono || e.Teléfono || '';
